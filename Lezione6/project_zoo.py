...
class Zoo:
    ...
    def add_animal(self, animal, fence):
        if fence.area >= animal.area:
            if animal.preferred_habitat == fence.habitat:
                ...
                print(f"{animal.name} added to {fence.habitat}.")
            else: 
                 print(f"There's enough space for {animal.name} but the habitat is wrong. ")
        else:
            print(f"{animal.name} cannot be added to {fence.habitat} because there's no space and the habitat is wrong. ")

    def remove_animal(self, animal, fence):
        if fence in self.fences:
            fence.remove_animal(animal)
            print(f"{animal.name} removed from {fence.habitat}.")
        else:
    ...
    def describe_zoo(self):
        ...
        print("\nFences:")
        for fence in self.fences:
            print(f"\n{fence.habitat}(area={fence.area}, temperature={fence.temperature}, habitat={fence.habitat})", end="\n\n")
            if fence.animals:
                print("with animals:\n")
...
class Animal:
    def __init__(self, name: str, species: str, age: int, height: int, width: int, preferred_habitat: str) -> None:
        self.name: str = name
...
class Fence:
    ...
    def add_animal(self, animal: Animal):
        if self.area > animal.area and animal.preferred_habitat == self.habitat:
    ...
    def feed_animals(self, animal: Animal):
        for animal in self.animals:
            if self.area >= (animal.area * 1.04):
                ...
                print(f"Not enough space in {self.habitat} to feed {animal.name}.")
    ...
    def clean(self) -> float:
        ...
        print(f"{self.habitat} has been cleaned in {cleaning_time} hours.")
